...
class VisualOdometryMono():
    ...
    def findMatches(self, frame_number, frame):
        ...
        try: # if there is no neighboor to pick two of them, this function will raise an error and we will catch it.
            matches = self.Matcher.knnMatch(old_descriptors, new_descriptors, k=2)
        ...
        dstPoints = np.float32([new_keypoints[m.queryIdx].pt for m in goodMatches]).reshape(-1, 1, 2) # points in the new frame
        ...
        if(self.visuals == 1):
            ...
            draw_params = dict(matchColor = (255,0,255), # draw matches in green color
                   ...
                   flags = 2)
            result = cv2.drawMatches(self.Frames[frame_number], new_keypoints, self.Frames[frame_number-1], old_keypoints, goodMatches,None,**draw_params)
